  ...
  return (
    <div className="space-y-6">
      ...
      <Field label="Description" error={errors.description}
        ...
        />
        <p className="text-right text-[11px] text-gray-400 mt-1">{form.description.length}/500</p>
      </Field>
...
const validate = (step, form) => {
  ...
  if (step === 1) {
    ...
    if (form.images.length === 0) errs.images   = 'Please upload at least one image'
  }
  if (step === 3) {
...
const AddVenue = () => {
  ...
  const buildPayload = () => ({
    ...
    rules: form.rules,
    operatingHours: form.bookingModel === 'hourly' ? {
